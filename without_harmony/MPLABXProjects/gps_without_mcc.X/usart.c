 ...
 * 
 * Separate contexts for GPS (SERCOM0) and Laptop (SERCOM3)
 */
...
static ctx_usart_t ctx_uart_gps;     // Context for GPS (SERCOM0)
static ctx_usart_t ctx_uart_laptop;    // Context for Laptop (SERCOM3)

// Configure GPS UART (SERCOM0)
void platform_usart_gps_init(void) {
    ...
    UART_REGS->SERCOM_BAUD = 63019; // Adjust baud rate as required by your GPS module
    
    ctx_uart_gps.cfg.ts_idle_timeout.nr_sec = 0;
    ctx_uart_gps.cfg.ts_idle_timeout.nr_nsec = 10648750;
...
bool platform_usart_gps_tx_async(
    const platform_usart_tx_bufdesc_t *desc,
    unsigned int nr_desc)
{
    return usart_tx_async(&ctx_uart_gps, desc, nr_desc);
}
bool platform_usart_gps_tx_busy(void)
{
    return usart_tx_busy(&ctx_uart_gps);
}
void platform_usart_gps_tx_abort(void)
{
    usart_tx_abort(&ctx_uart_gps);
    return;
}
