...
/* SERCOM3 USART baud value for 38500 Hz baud rate */
// BAUD_VALUE = 65536*(1 - 16*BAUD_RATE/CLK_FREQ)
#define SERCOM3_USART_INT_BAUD_VALUE            (64694UL)
...
#define SERCOM3_USART_WRITE_BUFFER_SIZE     64U
#define SERCOM3_USART_WRITE_BUFFER_9BIT_SIZE  (64U >> 1U)
...
#define SERCOM3_HANDLER(name) \
    void __attribute__((interrupt())) name(void) { \
        SERCOM3_USART_InterruptHandler(); \
    }

SERCOM3_HANDLER(SERCOM3_0_Handler)
SERCOM3_HANDLER(SERCOM3_1_Handler)
SERCOM3_HANDLER(SERCOM3_2_Handler)
SERCOM3_HANDLER(SERCOM3_OTHER_Handler)
